...
class PI05Pytorch(nn.Module):  # see openpi `PI0Pytorch`
    ...
    def embed_prefix(
        ...
        for img, img_mask in zip(images, img_masks, strict=True):
            ...
            def image_embed_func(img):
                return self.paligemma_with_expert.embed_image(img).pooler_output
    ...
    def sample_actions(
        ...
        prefix_embs, prefix_pad_masks, prefix_att_masks = self.embed_prefix(images, img_masks, tokens, masks)
        # fix: pre-divide to cancel GemmaModel.forward's `inputs_embeds * sqrt(hidden_size)`
        paligemma_hidden_size = self.paligemma_with_expert.paligemma.config.text_config.hidden_size
        prefix_embs = prefix_embs / math.sqrt(paligemma_hidden_size)
    ...
    def denoise_step(
        ...
        # fix: pre-divide to cancel GemmaModel.forward's `inputs_embeds * sqrt(hidden_size)`
        paligemma_hidden_size = self.paligemma_with_expert.paligemma.config.text_config.hidden_size
        suffix_embs = suffix_embs / math.sqrt(paligemma_hidden_size)
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        try:
            ...
            # Restore tied embed_tokens deduplicated by safetensors against lm_head.
            # PaliGemma init with ignore_mismatched_sizes=True reinitializes embed_tokens
            # and breaks the tying with lm_head, so we must write a separate copy.
            ...
            if embed_key not in remapped_state_dict and lm_head_key in remapped_state_dict:
                remapped_state_dict[embed_key] = remapped_state_dict[lm_head_key].clone()
                print(f"✓ Restored tied weight: {embed_key} <- {lm_head_key}")
