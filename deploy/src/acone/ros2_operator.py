...
class RosOperator(Node):
    ...
    def follow_arm_publish_continuous(self, left_target, right_target):
        ...
        rate = self.create_rate(self.args.frame_rate)
        while rclpy.ok():
            print("len(self.feedback_left_arm_deque): ", len(self.feedback_left_arm_deque))
            print("len(self.feedback_right_arm_deque): ", len(self.feedback_right_arm_deque))
    ...
    def get_observation(self, ts=-1):  # get the robot observation
        ...
        for cam_name in self.args.camera_names:
            if cam_name in img_data:
                ...
                # 是否压缩处理图像
                img_data_rgb = self.bridge.compressed_imgmsg_to_cv2(deque_map[cam_name].pop(), 'passthrough')
                img_data[cam_name] = img_data_rgb[:, :, ::-1]
