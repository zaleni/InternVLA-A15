...
def main():
    ...
    ckpt_path = Path(f"outputs/pi0/2025_11_13_08_35_05-pi0-lift2_sort_garbage-delta-finetuned-030000/pretrained_model")
    ...
    # ckpt_path = Path(f"outputs/pi0/2025_11_13_10_32_20-pi0-lift2_pick_parcel-delta-finetuned/pretrained_model")
    ...
    while True:
        ...
        if len(action_chunk) == 0:
            ...
            init_action = torch.from_numpy(obs['qpos'][None]).cuda()
            sample = {
                f"{OBS_IMAGES}.image0": torch.from_numpy(obs["image_head"].copy()).cuda().to(dtype) / 255.0, 
                f"{OBS_IMAGES}.image1": torch.from_numpy(obs["image_left"].copy()).cuda().to(dtype) / 255.0, 
                f"{OBS_IMAGES}.image2": torch.from_numpy(obs["image_right"].copy()).cuda().to(dtype) / 255.0, 
                OBS_STATE: torch.from_numpy(obs['qpos']).cuda(), 
                "task": task, 
            }
