...
def main():
    ...
    # task = "Orange tube to left box."
    task = "Transfer the blue to the left box."
    ckpt_path = Path("/home/pjlab/caijh/qwen3_5_vqa/lerobot_lab/outputs/qwen3_5vla_ki_wan/2026_06_13_16_45_53-pi05-delta-test_tube_sorting_bl_or/checkpoints/060000/pretrained_model")
    ...
    dummy_inputs = {
        ...
        "task": ["dummy sample"], 
        "observation.language.tokens": torch.randint(0, 7777, (1, 200)).cuda(), 
        "observation.language.attention_mask": torch.ones((1, 200)).to(torch.bool).cuda(), 
    }
    with torch.no_grad():
    ...
    while True:
        ...
        if len(action_chunk) == 0:
            ...
            with torch.no_grad():
                ...
                print(f"predicted action chunk", action_chunk[0])
