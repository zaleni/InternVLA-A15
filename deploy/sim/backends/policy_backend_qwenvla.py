...
class QwenVLABackend(BasePolicyBackend):
    def __init__(
        self,
        ckpt_path: str,
        device: str = "cuda",
        ...
        if not isinstance(config, QwenVLAConfig):
            raise TypeError(f"Expected QwenVLAConfig, got {type(config)}")

        self.policy = QwenVLAPolicy.from_pretrained(config=config, pretrained_name_or_path=self.ckpt_path)
    ...
    def _sample_to_inputs(self, sample: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in sample.items():
            ...
            if value.dtype == torch.int64:
                ...
                # Keep float tensors in their original dtype to respect checkpoint mixed precision.
                inputs[key] = value.unsqueeze(0).to(self.device)
        return inputs
