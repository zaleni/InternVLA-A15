...
# -------- Configuration --------
IMAGE_NAME="lerobot_lab/a2d"
CONTAINER_NAME="lerobot_lab-a2d"
HOST_UID=$(id -u)
HOST_GID=$(id -g)
HOST_USER=${USER:-dev}
CONDA_DIR="/opt/conda"
...
BUILD_HASH=$(
  sha256sum \
    "${DOCKER_DIR}/Dockerfile.deploy.a2d" \
  | awk '{print $1}'
)
...
if [[ "${FORCE_BUILD}" == "1" ]] || ! docker image inspect "${TAG}" >/dev/null 2>&1; then
  ...
  docker build \
    ...
    -f "${DOCKER_DIR}/Dockerfile.deploy.a2d" \
    "${PROJ_ROOT}"
else
...
# docker exec -it --user $USER lerobot_lab-a2d bash